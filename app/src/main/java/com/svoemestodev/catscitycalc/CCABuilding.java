...
public class CCABuilding extends CityCalcArea{
    ...
    public CCABuilding(CityCalc cityCalc, Area area, float x1, float x2, float y1, float y2, int[] colors, int[] ths) {
        ...
        this.isPresent = PictureProcessor.frequencyPixelInBitmap(this.bmpSrc, 0xFFFFFFFF) > 0.2f;
        if (this.isPresent) {
            this.mayX2 = PictureProcessor.frequencyPixelInBitmap(this.bmpSrc, this.colors[3]) > 0.01f || PictureProcessor.frequencyPixelInBitmap(this.bmpSrc, this.colors[4]) > 0.01f;
            this.isX2 = PictureProcessor.frequencyPixelInBitmap(this.bmpSrc, this.colors[4]) > 0.01f;
            this.buildingIsOur = PictureProcessor.frequencyPixelInBitmap(this.bmpSrc, this.colors[0]) > 0.01f;
            this.buildingIsEnemy = PictureProcessor.frequencyPixelInBitmap(this.bmpSrc, this.colors[1]) > 0.01f;
